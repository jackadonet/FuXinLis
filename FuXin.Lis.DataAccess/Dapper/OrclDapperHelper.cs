...
namespace FuXin.Lis.DataAccess.Dapper
{
    public class OrclDapperHelper
    {
        static string connStrRead = ConfigurationManager.ConnectionStrings["ORCL"].ConnectionString;
        static string connStrWrite = ConfigurationManager.ConnectionStrings["ORCL"].ConnectionString;
        ...
        public static T ExecuteReader<T>(string sql, object param = null, bool useWriteConn = false, IDbTransaction transaction = null)
        {
            using (IDbConnection conn = GetConnection(useWriteConn))
            {
                conn.Open();

                return conn.ExecuteScalar<T>(sql, param, commandTimeout: commandTimeout, transaction: transaction);
            }
        }
